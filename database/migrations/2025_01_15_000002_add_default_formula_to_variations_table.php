...
class AddDefaultFormulaToVariationsTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('variation', function (Blueprint $table) {
            $table->json('default_stock_formula')->nullable()->after('state')->comment('Per-variation default stock formula: {"value": float, "type": "percentage|fixed", "apply_to": "pushed|total"}');
