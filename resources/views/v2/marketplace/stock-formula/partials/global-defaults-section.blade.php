                            ...
                            <div class="d-flex justify-content-between align-items-start">
                                <div class="flex-grow-1">
                                    <h6 class="mb-2">
                                        ...
                                        @if($isBackMarket)
                                            <span class="badge bg-warning text-dark ms-2 formula-badge">Locked</span>
                                        @elseif($hasDefault)
                                    ...
                                    @else
                                        <p class="text-muted small mb-0">
                                            @if($isBackMarket)
                                                BackMarket is locked and cannot have a global default formula.
                                            @else
                                                No global default formula set for this marketplace.
                                            @endif
                                        </p>
                                    @endif
                                </div>
...
<script>
    ...
    function editGlobalDefault(marketplaceId) {
        // Prevent editing BackMarket (marketplace ID 1)
        if (marketplaceId == 1) {
            showAlert('BackMarket is locked and cannot have a global default formula', 'warning');
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    setTimeout(function() {
                        window.location.href = "{{ url('v2/marketplace/stock-formula') }}?tab=global";
                    }, 1000);
                } else {
    ...
    function deleteGlobalDefault(marketplaceId) {
        // Prevent deleting BackMarket (marketplace ID 1)
        if (marketplaceId == 1) {
            showAlert('BackMarket is locked and cannot have a global default formula', 'warning');
            return;
        }
