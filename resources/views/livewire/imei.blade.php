    ...
    @section('content')
        ...
        <div class="d-flex justify-content-between">
            ...
            <div class="p-2 d-flex justify-content-between">
                ...
                @endif
                <button class="btn btn-primary" onclick="printQz('{{ url('imei/print_label').'?stock_id='.$stock->id}}&minimal=1&autoprint=1', 'Sticker')" id="printBtn">Print Sticker</button>
                                    ...
                                    @foreach ($orders as $index => $item)
                                        ...
                                        <tr>
                                            ...
                                            <td style="width:240px" title="{{ $item->stock_id }}">
                                                    <strong class="text-danger">{{ $order->order_status->name ?? "Error finding order Status" }}</strong>
                                                @isset($item->stock->imei) {{ $item->stock->imei }}&nbsp; @endisset
                                                @isset($item->stock->serial_number) {{ $item->stock->serial_number }}&nbsp; @endisset
                                                @isset($item->admin_id) | {{ $item->admin->first_name }} |
                                                @else
                                ...
                                <tbody>
                                    ...
                                    @foreach ($stocks as $operation)
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ ($operation->stock->order->customer->last_name ?? "Error ")." | ".   ($operation->stock->order->reference_id ?? "Error") }}</td>
