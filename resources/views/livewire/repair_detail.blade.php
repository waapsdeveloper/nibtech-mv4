@extends('layouts.app')
    ...
    @section('content')
        ...
        <div class="d-flex justify-content-between" style="border-bottom: 1px solid rgb(216, 212, 212);">
            ...
            <h5>Reference: {{ $process->reference_id }} | Repairer: {{ $process->customer->first_name ?? null }} | Total Items: {{ $process->process_stocks->count() }} | Total € Price: €{{ amount_formatter($process->process_stocks->sum('price'),2) }}
                @if ($currency != 4)
                    | Total {{ $currency }} Price: {{ $currency.amount_formatter($process->process_stocks->sum('price') * $process->exchange_rate,2) }}
                @endif
                </h5>
            @if ($process->status == 3)
            <div>
                ...
                <a href="{{url('export_repair_invoice')}}/{{ $process->id }}/3" target="_blank"><button class="btn btn-sm btn-secondary">{{$process->currency_id->sign}} Invoice</button></a>
                @endif
            </div>
            @endif
                                ...
                                <tbody>
                                    ...
                                    @foreach ($last_ten as $p_stock)
                                        ...
                                        <tr>
                                            ...
                                            @if (session('user')->hasPermission('view_cost'))
                                            <td>{{ $currency.(amount_formatter($item->purchase_item->price ?? null,2)) }}</td>
                                            @endif
                                ...
                                <tbody>
                                    ...
                                    @foreach ($processed_stocks as $processed_stock)
                                        ...
                                        <tr class="{{ $danger }}">
                                            ...
                                            @if (session('user')->hasPermission('revert_repair_item') && $process->status == 2 && $processed_stock->updated_at->diffInDays() < 4)
