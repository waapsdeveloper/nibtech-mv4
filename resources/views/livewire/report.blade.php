        ...
        <div class="breadcrumb-header justify-content-between">
            ...
            <div>
                <div class="btn-group p-1" role="group">
                    <a href="{{url('report/purchase_report')}}" class="btn btn-link" target="_blank">Purchase Report</a>
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        headings.forEach((heading) => {
                            table += `<td>`;
                            Object.values(data.currency_ids).forEach((key) => {
                                if (b2b_sales_data[heading] && b2b_sales_data[heading][key] && b2b_sales_data[heading][key]['average_price'] != 0) {
                        ...
                        headings.forEach((heading) => {
                            table += `<td>`;
                            Object.values(data.currency_ids).forEach((key) => {
                                if (b2b_sales_data[heading] && b2b_sales_data[heading][key] && b2b_sales_data[heading][key]['total_sales'] != 0) {
                        ...
                        headings.forEach((heading) => {
                            table += `<td>`;
                            Object.values(data.currency_ids).forEach((key) => {
                                if (b2c_sales_data[heading] && b2c_sales_data[heading][key] && b2c_sales_data[heading][key]['average_price'] != 0) {
                        ...
                        headings.forEach((heading) => {
                            table += `<td>`;
                            Object.values(data.currency_ids).forEach((key) => {
                                if (b2c_sales_data[heading] && b2c_sales_data[heading][key] && b2c_sales_data[heading][key]['total_sales'] != 0) {
                            ...
                            <table class="table table-bordered table-hover text-md-nowrap">
                                ...
                                table += `
                                    ...
                                    <tr>
                                        ...
                                        <td colspan="${Object.keys(data.currency_ids).length}">€${b2bReturn.b2b_return_sum}</td>
                                        <td colspan="${Object.keys(data.currency_ids).length}">€${b2bReturn.b2b_return_total}</td>
                                    </tr>
                                    <tr>
                                        <td>B2C Returns</td>
                                ...
                                if (typeof data.currency_ids === 'object') {
                                    ...
                                    Object.values(data.currency_ids).forEach((key) => {
                                        table += `
                                            <td title="Price - Charges - Cost - Repair Cost">
                                                ${b2cReturn.b2c_return_totals_exchanged[key] ? `${currencies[key]}${b2cReturn.b2c_return_totals_exchanged[key]}` : ''}
                                            </td>
                                        `;
                                    });
                                }
