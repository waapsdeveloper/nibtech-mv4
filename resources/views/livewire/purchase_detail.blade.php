                    ...
                    <table class="table table-bordered table-hover mb-0 text-md-nowrap">
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <th title="{{ $sold_total['total_quantity'] > 0 ? $sold_total['total_cost']/$sold_total['total_quantity'] : 0 }}" ><b>€{{ amount_formatter($sold_total['total_cost'],2) }}</b></th>
                                <th title="{{ $sold_total['total_quantity'] > 0 ? $sold_total['total_repair']/$sold_total['total_quantity'] : 0 }}"><b>€{{ amount_formatter($sold_total['total_repair'],2) }}</b></th>
                                <th title="{{ $sold_total['total_quantity'] > 0 ? $sold_total['total_charge']/$sold_total['total_quantity'] : 0 }}"><b>€{{ amount_formatter($sold_total['total_charge'],2) }}</b></th>
                                <th title="{{ $sold_total['total_quantity'] > 0 ? $sold_total['total_price']/$sold_total['total_quantity'] : 0 }}"><b>€{{ amount_formatter($sold_total['total_price'],2) }}</b></th>
                                <th title="{{ $sold_total['total_quantity'] > 0 ? $sold_total['total_profit']/$sold_total['total_quantity'] : 0 }}"><b>€{{ amount_formatter($sold_total['total_profit'],2) }}</b></th>
                            </tr>
                        </tfoot>
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($testing_list as $id => $testing_list_grouped)
                                                ...
                                                @foreach ($testing_list_grouped as $row)
                                                    ...
                                                    @if ($row['imei'] != null)
                                                        <input type="hidden" name="imeis[]" value="{{$row['imei']}}" form="order_testing_list_{{$j}}">
                                                    @else
                                                        <input type="hidden" name="imeis[]" value="{{$row['serial_number']}}" form="order_testing_list_{{$j}}">
                                                    @endif
                                                    <tr>
                                                        <td>{{ ++$i }}</td>
                                                        <td>{{ $row['imei'] }}</td>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            ...
                            >
                                <div class="card-header pb-0">
                                    @php
                                        isset($variation->product_id) && isset($products[$variation->product_id]) ? $product = $products[$variation->product_id] : $product = "MODEL NOT FOUND";
                                        ...
                                        <table class="table table-bordered table-hover mb-0 text-md-nowrap">
                                            ...
                                            <tbody>
                                                ...
                                                @endphp
                                                    <tr>
                                                        <td>{{ $i }}</td>
                                                        <td data-stock="{{ $item->id }}" class="{{$class}}" title="{{ $item->id}}">{{ $item->imei.$item->serial_number }}
                                                            @if ($class != "")
                                                ...
                                                @foreach ($sold_stocks as $stock)
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            isset($variation->grade)?$grade = $grades[$variation->grade] ?? null:$grade = null;
            ...
            function PrintElem(elem)
{
            ...
            function loadProductRepairDetails(orderId, productId) {

                fetch(`{{ url('purchase') }}/purchase_model_color_graded_repair/${orderId}/${productId}`)
                .then(response => response.json())
                .then(productss => {
                    console.log(productss);
                    // Render the product details
