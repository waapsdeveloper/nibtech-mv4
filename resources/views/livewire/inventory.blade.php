...
@section('content')
    ...
    <form action="" method="GET" id="search">
        <div class="row">
            ...
            <div class="col-md col-sm-2">
                <select name="category" class="form-control form-select" data-bs-placeholder="Select Category"
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" @if (isset($_GET['category']) && $category->id == $_GET['category']) {{ 'selected' }} @endif>
            ...
            <div class="col-md col-sm-2">
                <select name="brand" class="form-control form-select" data-bs-placeholder="Select Brand"
                    ...
                    @foreach ($brands as $brand)
                        <option value="{{ $brand->id }}" @if (isset($_GET['brand']) && $brand->id == $_GET['brand']) {{ 'selected' }} @endif>
                            {{ $brand->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md col-sm-3">
                ...
                <datalist id="product-menu">
                    ...
                    @foreach ($products as $id => $model)
                        <option value="{{ $id }}" @if (request('product') != null && $id == request('product')) {{ 'selected' }} @endif>
                            {{ $model }}</option>
                    @endforeach
                </datalist>
            </div>
            ...
            <div class="col-md col-sm-2">
                <div class="form-floating">
                    <input type="text" name="sku" value="{{ Request::get('sku') }}" class="form-control"
                        data-bs-placeholder="Select SKU">
            ...
            <div class="col-md col-sm-2">
                <select name="storage" class="form-control form-select">
                    ...
                    @foreach ($storages as $id => $name)
                        <option value="{{ $id }}" @if (request('storage') != null && $id == request('storage')) {{ 'selected' }} @endif>
                            {{ $name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md col-sm-2">
                <select name="color" class="form-control form-select">
                    ...
                    @foreach ($colors as $id => $name)
                        <option value="{{ $id }}" @if (isset($_GET['color']) && $id == $_GET['color']) {{ 'selected' }} @endif>
                            {{ $name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-5 col-sm-2">
                <select name="grade[]" class="form-control form-select select2" multiple>
                    ...
                    @foreach ($grades as $id => $name)
                        <option value="{{ $id }}" @if (isset($_GET['grade']) && in_array($id, $_GET['grade'])) {{ 'selected' }} @endif>
                            {{ $name }}</option>
                    @endforeach
                </select>
            </div>
    ...
    @if (session('user')->hasPermission('view_inventory_summery') &&
            request('summery') != 1 &&
            session('user')->hasPermission('view_cost') &&
            $stocks->count() > 0)
        <div class="" id="vendor_wise_average"></div>
    @endif
    <div class="d-flex justify-content-between">
        <div>

            <a href="{{ url('inventory') }}?status=3&grade[]=1&grade[]=2&grade[]=3&grade[]=5&grade[]=7&grade[]=9"
                class="btn btn-link @if (request('status') == 3 && request('grade') == [1, 2, 3, 5, 7, 9]) bg-white @endif ">RTG</a>
            <a href="{{ url('inventory') }}?status=3"
                class="btn btn-link @if (request('status') == 3) bg-white @endif ">Active</a>
            <a href="{{ url('inventory') }}?status=2"
                class="btn btn-link @if (request('status') == 2) bg-white @endif ">Pending</a>
            <a href="{{ url('inventory') }}"
                class="btn btn-link @if (request('status') == null) bg-white @endif">All</a>
            @if (session('user')->hasPermission('view_inventory_summery'))
                <button class="btn btn-link  @if (request('summery') == 1) bg-white @endif" type="submit"
                    form="summery">Summery</button>
                                    ...
                                    @foreach ($stocks as $index => $stock)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @endif
                                                @if ($stock->latest_listing)
                                                    &nbsp; {{ $stock->latest_listing->process->reference_id }}
                                                @endif
    ...
    <script>
        ...
        function ExportExcel(elem) {
            ...
            [
                ...
                <head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>Sheet1</x:Name><x:WorksheetOptions></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head>
