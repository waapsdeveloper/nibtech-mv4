...
class Repair extends Component
{
    ...
    public function render()
    {
        ...
        if (session('user')->hasPermission('view_repair_history') && request('history') == 2) {
            $process_stocks = Process_stock_model::where('status',3)->whereHas('process', function($q){
                $q->where('process_type_id',9)
                ->when(request('repairer_id'), function ($q) {
                    return $q->where('customer_id', request('repairer_id'));
                });
    ...
    public function export_repair_invoice($process_id, $invoice = null)
    {
        ...
        if(request('packlist') == 1){
            ...
            // $process->customer->first_name.'_'.
            $process->reference_id.'_'.$process->description.'_'.$process->process_stocks->count().'pcs.xlsx');
        }else{
