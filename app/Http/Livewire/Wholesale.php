...
class Wholesale extends Component
{
    ...
    public function export_bulksale_invoice($order_id, $invoice = null)
    {
        ...
        if(request('packlist') == 1){
            $pdf->SetTitle('Bulksale Packlist '.$order->reference_id.' '.$order->customer->company.' '.$order_items_2->sum('total_quantity').' items');
            ...
            $pdf->SetTitle('Bulksale Invoice '.$order->reference_id.' '.$order->customer->company.' '.$order_items_2->sum('total_quantity').' items');
