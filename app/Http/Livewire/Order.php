...
class Order extends Component
{
    ...
    public function render()
    {
        ...
        ->when(request('missing') == 'scan', function ($q) {
            return $q->whereIn('status', [3,6])->whereNull('scanned')->where('processed_at', '<=', now()->subHours(12));
        })
        ...
        if($orderObj->state == 3){
            foreach($imeis as $i => $imei){
                ...
                if($stock[$i]->created_at->diffInDays() < 20 && !session('user')->hasPermission('allow_sell_new_stock')){
                    $stocks = Stock_model::where('variation_id',$variant->id)->where('status',1)->where('order_id','<' , $stock[$i]->order_id)->where('updated_at','<',now()->subDays(5))->get();
