...
class IMEI extends Component
{
    ...
    public function render()
    {
        ...
        if (request('imei')) {
            ...
            $inventory_verifications = Process_stock_model::where('stock_id', $stock_id)->whereHas('process', function ($query) {
            ...
            // print_r($last_item);
            $orders = Order_item_model::where('stock_id', $stock_id)->with(['order'])->orderBy('id','desc')->get();
            $data['stock'] = $stock;
            $data['orders'] = $orders;
            // dd($orders);
