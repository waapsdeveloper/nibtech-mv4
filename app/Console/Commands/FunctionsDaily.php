...
class FunctionsDaily extends Command
{
    ...
    private function add_order_charge(){
        $orders = Order_model::whereIn('status',[3,5,6])->where('order_type_id',3)->whereNull('charges')->limit(100)->get();
        ...
        foreach($orders as $order){
            ...
            if($order->status == 5){
                ...
                $order_charge = new Order_charge_model();
                $order_charge->order_id = $order->id;
                $order_charge->charge_value_id = $charge->current_value->id;
