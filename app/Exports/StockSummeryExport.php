        ...
        foreach($result as $category => $cat){
            ...
            foreach($cat as $brand => $datas){
                ...
                foreach($datas as $data){
                    ...
                    $pdf->MultiCell(8, 0, $i, 1, 'L', false, 0, '', '', true);
                    $variationName = $this->ellipsize($data['model'], 40);
                    // $pdf->Cell(80, 0, $variationName, 1);
                    $pdf->MultiCell(80, 0, $variationName, 1, 'L', false, 1, '', '', true);
                    $pdf->MultiCell(12, 0, $data['stock_count'], 1, 'C', false, 1, '', '', true);
                    $pdf->MultiCell(18, 0, number_format($data['average_cost'],2), 1, 'C', false, 1, '', '', true);
                    $pdf->MultiCell(18, 0, $this->bold($data['graded_average_cost'][1] ?? 0), 1, 'C', false, 1, '', '', true);
                    $pdf->MultiCell(18, 0, $this->bold($data['graded_average_cost'][2] ?? 0), 1, 'C', false, 1, '', '', true);
                    $pdf->MultiCell(18, 0, $this->bold($data['graded_average_cost'][3] ?? 0), 1, 'C', false, 1, '', '', true);
                    $pdf->MultiCell(18, 0, $this->bold($data['graded_average_cost'][5] ?? 0), 1, 'C', false, 1, '', '', true);
